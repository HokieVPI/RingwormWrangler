...
uint32_t motor1_accel = 25000; // Acceleration rate in counts/sec/sec (check ****)
int32_t motor1_target_speed = 7062; // Target speed in counts/sec (signed)
...
void loop() {
  ...
  if (millis() - lastChange > 5000) { // Change every 5 seconds
      ...
      if (motor1_target_speed > 0) {
          motor1_target_speed = -7062; // Switch to reverse speed
      } else {
          motor1_target_speed = 7062; // Switch to forward speed
      }

/**
  Two-Way Ranging Controller with AoA 2D Positioning for Arduino Stella
  Name: stella_uwb_twr_aoa_2d.ino
...
// In practice, you might want to match by MAC address if available in twr[j]
  ...
  if (rangingData.measureType() == (uint8_t)uwb::MeasurementType::TWO_WAY) {
    ...
    for (int j = 0; j < rangingData.available(); j++) {
      ...
      if (twr[j].status == 0 && twr[j].distance != 0xFFFF) {
        ...
        // Extract AoA (as per user's example: float angle_degrees = rangingData.aoa;)
        float angle_degrees = rangingData.aoa;
        ...
        // Only calculate if measurements are recent (within 100ms of each other)
    ...
    if (measurements_ready) {
      ...
      // Reset measurements after calculation (optional - keep for next round)
...
void setup() {
  ...
  Serial.println("========================================");
  Serial.println("Arduino Stella - TWR AoA 2D Positioning");
  Serial.println("========================================");
  ...
  // Create sessions for both anchors
  Serial.println("Creating sessions for both anchors...");
  
  // Session 1: Anchor 1 (MAC 0x1111)
  UWBMacAddress dstAddr1(UWBMacAddress::Size::SHORT, ANCHORS[0].mac);
  UWBTracker tracker1(0x11223344, srcAddr, dstAddr1);
  UWBSessionManager.addSession(tracker1);
  tracker1.init();
  tracker1.start();
  Serial.println("Session 1 (Anchor 1) started");

  // Session 2: Anchor 2 (MAC 0x3333)
  UWBMacAddress dstAddr2(UWBMacAddress::Size::SHORT, ANCHORS[1].mac);
  UWBTracker tracker2(0x11223344, srcAddr, dstAddr2);
  UWBSessionManager.addSession(tracker2);
  tracker2.init();
  tracker2.start();
  Serial.println("Session 2 (Anchor 2) started");
